...
//        - service_activities(activity_id, client_id, program_id, staff_id, activity_type, activity_date, notes, is_flagged).
//
//        case_statuses.status may contain only these values:
//        'Active', 'On Hold', 'Pending Review', or 'Completed'.
//        Match those values exactly.
//
//        Return EXECUTE only for answerable operational questions; questions directly
//        related to the given schema
//
//        For EXECUTE, return one read-only parameterized SQL query using ? placeholders and
...
//        relevant primary key and group by the primary key; never return an aggregate-only
